...
The DAL **implements** the interfaces (`Protocol`s) declared in `app/bl/ports/`:
...
```
app/dal/
├── postgres.py                 connection factory
├── layers_repository.py        implements LayersRepository (public.layers)
├── feedback_repository.py      persists 👍/👎 feedback (no port — called directly)
├── providers/
│   ├── mqs*.py                 MQS (Moria Query Service) adapter
│   ├── cubes*.py                Cubes (time-varying entity) adapter
│   ├── tyche*.py                Tyche (Our Forces) adapter
│   └── registry.py              InMemoryProviderRegistry
└── llm/
    └── openai_client.py        OpenAI-compatible LLMClient + supporting collaborators
```

## Port interfaces implemented here (declared in `bl/ports/`)
...
| File | Class | Role |
|---|---|---|
| `mqs.py` | `MqsProvider` | Orchestrator |
| `mqs_source.py` | `MqsSource` | Parses `mqs://layer/<id>`; resolves the temporal field tag |
| `mqs_filter_builder.py` | `MqsFilterBuilder` | Builds the POST filter body (`geo_polygon`/`geo_bounding_box`, `simple_operators.match`); quadrant-splits a geometry |
| `mqs_gateway.py` | `MqsGateway` | HTTP boundary: GET/POST, pagination, `/MoriaProject/{id}/EntityInfo/{entity_id}` detail fetch, layer listing |
| `mqs_entity_stream.py` | `MqsEntityStream` | Adaptive quadtree splitting, cross-tile dedup by `entity_id`, concurrent detail enrichment, per-layer cap |
| `mqs_entity_mapper.py` | `MqsEntityMapper` | Normalizes entity JSON variants (`property_list` schema-agnostic parsing, WKT geometry) into GeoDataFrame records |
| `mqs_schema_builder.py` | `MqsSchemaBuilder` | Infers `LayerSchema` from enriched sample entities |
...
## Cubes provider — `providers/cubes*.py`
...
| File | Class | Role |
|---|---|---|
| `cubes.py` | `CubesProvider` | Orchestrator |
| `cubes_source.py` | `CubesSource` | Parses `cubes://db/<dbname>`, `query_mode`, `param_<name>=<value>` resolved-parameter map |
| `cubes_client_factory.py` | `CubesClientFactory` | Authenticated `httpx.Client` (Bearer token, TLS verify) |
| `cubes_metadata_gateway.py` | `CubesMetadataGateway` | `GET /cube/v1/{cube}` metadata (cached per db), `POST /cube/v1/{cube}/autocomplete/{param}` (never cached) |
| `cubes_parameter_loader.py` | `CubesParameterLoader` | Hydrates name-only parameter entries via `GET /cube/v1/{cube}/parameters/{name}` |
| `cubes_query_builder.py` | `CubesQueryBuilder` | Builds request bodies (`.match`/`.not`, polygon, `Location`), chunk splitting, required-parameter validation |
| `cubes_gateway.py` | `CubesGateway` | POSTs rows, recursive capped-result recovery (spatial then temporal chunking) |
| `cubes_schema_mapper.py` | `CubesSchemaMapper` | Maps metadata/params/response rows into `LayerSchema`/`LayerField`/`LayerParameter`/GeoDataFrame; dedup |
...
## Tyche provider — `providers/tyche*.py`

Simpler single-source provider (`tyche://ourforces` only).

| File | Class | Role |
|---|---|---|
| `tyche.py` | `TycheProvider` | Orchestrator; validates `source_url == tyche://ourforces` |
| `tyche_gateway.py` | `TycheGateway` | `POST /coordinate/v1/ourforces`, `pageTracker` pagination, dedup, safety cap |
| `tyche_query_builder.py` | `TycheQueryBuilder` | Builds `eventTime.match` window, `location.match` WKT, `pageTracker` |
| `tyche_feature_mapper.py` | `TycheFeatureMapper` | Parses varied geometry encodings; row dedup by `id` |
| `tyche_schema_builder.py` | `TycheSchemaBuilder` | Fixed field list (`eventTime`, `callSign`, `forceType`, `unit`, `netId`, ...) + discovered extras |
...
## Layers repository & Postgres — `layers_repository.py`, `postgres.py`
...
`PostgresConnection.connect(store)` (`postgres.py`) — `psycopg` connection with
...
## Feedback repository — `feedback_repository.py`

**`PostgresFeedbackRepository`** — persists 👍/👎 from the UI. No corresponding
`bl/ports` interface — called directly from `feedback_router.py`, not through DIP.
...
- `mqs.py` and `cubes.py` keep a few module-level compatibility aliases at the bottom
