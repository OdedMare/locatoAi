...
**Compose:** Use the latest subject-chain step as input. Set a requested corridor width; otherwise omit it so the contract supplies the default. Each named endpoint requires its complete filter. Add both target layers to `context_layers`.
