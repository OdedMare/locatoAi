...
**Compose:** Apply the requested `temporal_filter` first and use the schema's declared `entity` and `time` roles. Never collapse observations before or after this operation. `together` aligns observations in space and time; use a zero movement threshold only for staying together. `same_destination` compares final positions and arrival times. `same_time` compares movement intervals without requiring the same location. `same_place_different_times` compares visits separated in time.
