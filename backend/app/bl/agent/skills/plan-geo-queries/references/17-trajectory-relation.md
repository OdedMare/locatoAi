...
**Do not use when:** The query concerns one entity's direction (`movement_direction`), one entity leaving and returning (`round_trip`), only the newest observation (`latest_per_entity`), or a static same-layer group (`cluster`). Do not collapse observations before this operation.
