...
**Compose:** Apply a requested `temporal_filter` first. Use `any` for movement without a bearing; it measures total path length. Compass directions use dominant first-to-last displacement. Use the layer schema's declared `entity` and `time` roles, and omit an unstated movement threshold so the contract supplies the default.
