...
**Compose:** Apply `temporal_filter` over the same explicit ISO interval first and use the schema's declared `entity` and `time` roles. Never guess a missing date, AM/PM, timezone, or definition of “night.” `departed` requires later movement away from the inferred origin. `round_trip` additionally requires an intermediate observation and a finish near that origin.
