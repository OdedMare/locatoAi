...
**Compose:** Use the latest subject-chain step as input. `count` is the user's result limit, not an aggregate. A named reference entity requires the complete target filter. Add the reference layer to `context_layers`.
