...
- Moving entities use only the schema's declared `entity` and `time` roles. Apply temporal_filter only for a requested window. Apply latest_per_entity after static/spatial filtering when the result means distinct current entities, and before clustering or counting them. Clarify if an operation needs a role the schema does not declare.
