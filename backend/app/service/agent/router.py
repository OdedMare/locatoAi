...
from app.service.agent.select_layers_request import SelectLayersRequest
from app.service.agent.select_layers_response import SelectLayersResponse
...
def select_layers(body: SelectLayersRequest, request: Request) -> SelectLayersResponse:
    ...
    _log(request, body.query, selection, timing_ms)
    return SelectLayersResponse(
        layers=[
            SelectedLayer(id=item.id, name=item.name, tags=item.tags)
...
router.add_api_route(
    ...
    select_layers,
    methods=["POST"],
    response_model=SelectLayersResponse,
)
