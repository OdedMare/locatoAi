...
class RankingRouter:
    @staticmethod
    def rank(body: RankingRequest, request: Request) -> RankingResult:
        boundaries = RankingRouter._boundaries(body)
        ...
        RankingRouter._log(request, result)
        return result
    ...
    def _validate_boundaries(boundaries):
        if boundaries.is_empty or not boundaries.is_valid:
            raise HTTPException(
                status_code=422, detail="Ranking boundaries must be valid"
            )
...
rank_area = RankingRouter.rank
router.add_api_route(
    "/api/ranking", rank_area,
    methods=["POST"], response_model=RankingResult,
)
