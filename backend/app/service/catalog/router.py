...
def list_layers(request: Request) -> LayersResponse:
    ...
    return LayersResponse(
        layers=[catalog_layer(layer) for layer in layers],
        count=len(layers),
    )
...
def list_remote_mqs_layers(request: Request) -> RemoteMqsLayersResponse:
    ...
    return RemoteMqsLayersResponse(
        layers=[_remote_layer(layer) for layer in layers],
...
def activate_tyche(request: Request) -> CatalogLayer:
    ...
    return catalog_layer(activated)

def create_layer(
    body: CreateLayerRequest, request: Request
) -> CatalogLayer:
    layer = _new_layer(body)
    try:
        created = request.app.state.catalog.add_layer(layer)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc))
    return catalog_layer(created)
...
def generate_layer_metadata(
    ...
    result = generator.generate(
        ...
        sample_geometry=_sample_geometry(body),
    )
...
def _new_layer(body: CreateLayerRequest) -> LayerMeta:
    return LayerMeta(
        id=str(uuid4()), name=body.name.strip(),
        description=body.description.strip(),
        tags=clean_tags(body.tags, 20),
        ...
        profiles=clean_profiles(body.profiles),
        source_url=normalized_source(
            body.provider, body.source_url,
            package_parameters=body.package_parameters,
            package_query=body.package_query,
