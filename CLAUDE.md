...
**Agent loop:** planning has up to three `sample_field` calls and one validation correction. Execution emits per-step counts. Zero rows permit one diagnosis/replan/re-execution. `preserves_constraints` rejects revisions that remove or widen filters, time, geography, distances, counts, targets, `netId` identity, or movement thresholds. Never add an unbounded loop or arbitrary SQL/HTTP tool. `llm_diet_mode` defaults on and dynamically selects compact select/build prompts, compact schemas/tool samples, and a 1,200-token completion cap; the full profile remains available in Settings. GeoQueryPlan operation guidance has one source per operation under `backend/app/bl/agent/skills/plan-geo-queries/references/`; both build profiles render that shared catalog and must remain operation-complete.
...
- `bl/plan/` — **GeoQueryPlan is the core contract**: an 18-member discriminated union, including single-entity direction/origin movement and multi-entity trajectory relations. Moving operations use schema-backed identity/time fields (`netId`/`eventTime` remain Cubes defaults). Semantic validation enforces earlier references, catalog IDs, complete target filters, required boundaries, final output ordering, and terminal count.
