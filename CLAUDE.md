...
**Where we are:** the whole MVP loop works end-to-end — NL query (Hebrew/English) → selected layers → GeoQueryPlan → executed features drawn on the map. Milestone 2 of the MVP guide is done (verified live: directional, near, temporal+attribute, within_geometry with drawn boundaries, and ambiguous→Hebrew clarify). **The main model is Gemma 4 31B via Ollama cloud** (`llm_model: gemma4:31b-cloud`, `llm_base_url: http://pghost:11434/v1` in runtime-settings — no API key; the LLM client is key-optional when a base_url is set; requires the host to be `ollama signin`-ed once). Selection eval: `docker exec ailocator-backend python scripts/eval_select_layers.py` (20/20). **Clarify questions are ALWAYS Hebrew** (product decision — enforced in both prompts and the `_FALLBACK_CLARIFY` constants). Plan building policy: validate → retry once with the error appended → clarify (`bl/agent/build_plan.py`); before committing, the model may run up to 2 **`sample_field` tool rounds** (JSON-protocol tool, separate budget from the retry) to pull up to 20 distinct values of a field via `CatalogService.sample_field` → `Provider.sample_field_values`; rounds surface as `tool_calls` in the response and in the UI agent panel. **The MQS (Moria Query Service) provider is live** (`dal/providers/mqs.py`, provider key `mqs`): catalog rows store `source_url = "mqs://layer/{layerId}"`; the base URL is the `mqs_base_url` setting (⚙ panel, nullable, read per call — unset → clear 502); fetch-all-filter-locally via paginated `GET /MoriaProject/{id}/Entities` (page 1000, hard cap 50k → error), lenient response parsing (no live MQS existed when written — all adaptation points are inside `mqs.py`). `POST /api/layers/sync-mqs` (button in the UI layers panel) upserts the MQS layer inventory into the catalog keyed on `(provider, source_url)` — re-syncs preserve tags; rerun `enrich_layer_tags.py` after syncing new layers. **`mqs` is the only production provider** — `MockArcgisProvider` (`dal/providers/arcgis_mock.py`) is a test fixture only (used by `tests/conftest.py`), never registered in `main.py` or `scripts/eval_select_layers.py`; new catalog rows default to `provider="mqs"` in both the API and the UI add-layer form. **Next candidates:** multi-turn clarify (conversation history in the request), client_now/tz, MQS spatial-filter pushdown, SSE streaming.
...
- `dal/` — `layers_repository.py` (the only SQL; incl. `upsert_layer` keyed on provider+source_url, tags preserved on update) + `providers/mqs.py` (MQS REST adapter — the only provider registered in production; see "Where we are") + `providers/arcgis_mock.py` (serves `data/*.geojson`; test fixture only, used by `tests/conftest.py`, never wired in `main.py`) + `llm/openai_client.py` (JSON-mode chat client behind the `LLMClient` port: reads model/key/base_url from the settings store on every call, strips ``` fences, retries once on bad JSON, falls back when a server rejects `response_format`).
...
**Catalog UI:** `LayersPanel` browses/searches the Postgres layer catalog and contains the add-layer form. `catalogService.ts` mirrors `catalog_router.py`: `GET /api/layers` lists metadata and `POST /api/layers` inserts a generated UUID plus name/description/tags/provider/source URL through `CatalogService` and `LayersRepository`. With the current mock provider, a newly added ArcGIS row only has executable features when its source URL slug matches `backend/data/<slug>.geojson`; arbitrary remote ArcGIS fetching is still a future adapter.
